...
import {
  ...
  InboundGitCreateNewVersionMessage,
  ...
  async onMessage(message: string) {
    ...
    try {
      switch (inboundMessage.type) {
        ...
        case InboundMessage.Type.UPLOAD: {
          // Protected message type
          if (!this.isController) {
            return;
          }

          const uploadMessage = <InboundUploadFileMessage> inboundMessage;
          if (uploadMessage.uploadFile) {
            await this.filesystemManager.upload(uploadMessage.uploadFile.path, uploadMessage.uploadFile.body);
        ...
        case InboundMessage.Type.SAVE_FILE: {
          ...
          if (saveFileMessage.saveFile) {
            const { path, options } = saveFileMessage.saveFile;
            await this.fileManager.save(path);

            if (options && options.commitImmediately) {
              const filePath = path.split('!')[0];
              // console.log(filePath, options.commitMessage);
              await this.gitManager.commitAndPush(filePath, options.commitMessage || 'empty commit');
            }
          }
          break;
        }
        ...
        case InboundMessage.Type.PTY_RELOAD: {
          // Protected message type
          if (!this.isController) {
            return;
          }
        ...
        case InboundMessage.Type.GIT_LOAD_STATUS: {
          if (!this.isController) {
            return;
          }
        ...
        case InboundMessage.Type.GIT_CREATE_NEW_VERSION: {
          // Protected message type
          if (!this.isController) {
            return;
          }

          const gitCreateNewVersionMessage = <InboundGitCreateNewVersionMessage> inboundMessage;
          if (gitCreateNewVersionMessage.gitCreateNewVersion) {
            const { message } = gitCreateNewVersionMessage.gitCreateNewVersion;
            this.gitManager.createNewVersion(message);
          }
          break;
        }
      }
